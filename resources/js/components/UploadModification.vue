<template>
<div class="ml-2">
    <button class="md-button" @click="$refs.csv_modifications.click()">
        ...
        <span>Импортировать модификации</span>
    </button>
    <input id="csv_modifications" type="file" ref="csv_modifications" class="hidden" multiple v-on:change="handleModificationsCsv()" />
    ...
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        <v-card dark>
            <v-toolbar dark color="black">
                ...
                <v-toolbar-title>Загружаемый файл:<span v-for="name in csvModificationsFileName"> {{ name }} </span></v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items v-if="disableAllFields && (modificationsLengthUploaded != modificationsBody.length)" class="d-flex align-center mr-4">
                    <span class="mr-4">
                        Загружено {{ modificationsLengthUploaded }} из {{ modificationsBody.length }}
                    </span>
                    <span class="mr-2">
                    ...
                    {{ ((modificationsLengthUploaded/modificationsBody.length)*100).toFixed(2) + '%' }}
                </v-toolbar-items>
                <v-toolbar-items v-if="modificationsFinished" class="d-flex align-center mr-4">
                ...
                <v-btn v-if="!disableAllFields" @click="uploadModifications()" :disabled="error || disableAllFields">
                ...
                <v-btn v-if="error == false && disableAllFields && (modificationsLengthUploaded != modificationsBody.length)" @click="stopLoad()">
            ...
            <div dark color="black">
                <v-chip v-if="(headClass(head[0].text) == 'error')" v-for="(head, index) in modificationsHead" :key="index" class="mr-2">
                    <v-avatar class="teal">{{ head[0].text.charAt(0).toUpperCase() }}</v-avatar>{{ head[0].text }}
                </v-chip>
            </div>
            <v-progress-linear color="green lighten-2" background-color="blue lighten-1" v-model="((modificationsLengthUploaded/modificationsBody.length)*100).toFixed(2)"></v-progress-linear>
            <v-list class="pa-0 ma-0">
                <div class="csv-table-wrap">
                    <div class="csv-table-product">
                        <div class="head">
                            <div v-for="(head, index) in modificationsHead">
                                <v-autocomplete no-data-text="Не найдено" :class="headClass(head[0].text)" dark v-model="head[0].text" :items="modificationsPreloadedHead" item-text="name" @change="error = true"></v-autocomplete>
                            </div>
                        </div>
                        <div class="body">
                            <div class="content">
                                <div v-for="body in modificationsBody" class="product">
                                    <div :class="modification.error ? 'error' : modification.loading" v-for="(modification, index) in body" :title="modification.text">
                                        <input :disabled="disableAllFields" type="text" v-model.lazy="modification.text" @keyup="error = true" />
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            checking: false,
            csvModifications: [],
            csvModificationsFileName: [],
            modificationsHead: [], // Array
            modificationsBody: [], // Array of objects
            modificationsLengthUploaded: 0,
            preload: false,
            loadedKB: 0,
            filesizes: 0,
            dialog: false,
            ...
            modificationsFinished: false,
        }
    },
    computed: {
        modificationsPreloadedHead() {
            let list = [
                {
                    name: 'Наименование',
                    disabled: false
            ...
            this.modificationsHead.forEach(
                (item) => {
                    list.forEach(
                        (list_item, index) => {
    ...
    watch: {
        modificationsLengthUploaded() {
            if (this.modificationsLengthUploaded == this.modificationsBody.length) {
                this.disableAllFields = false;
                this.modificationsLengthUploaded = 0;
                for (const item of this.modificationsBody) {
                    item.forEach(row => row.loading = 'waiting');
                }
                this.modificationsFinished = true;
            }
        },
    },
    methods: {
        ...
        handleModificationsCsv() {
            this.csvModifications = [];
            this.filesizes = 0;
            let uploadedFiles = this.$refs.csv_modifications.files;
            for (var i = 0; i < uploadedFiles.length; i++) {
                this.csvModifications.push(uploadedFiles[i]);
                this.filesizes += uploadedFiles[i].size;
            }
            this.onSubmitModifications();
        },
        checkForErrors() {
            this.checking = true;
            axios.post('/api/csv/check/modifications', {
                modificationsHeader: this.modificationsHead,
                modifications: this.modificationsBody,
            }).then(
                (success) => {
                    this.checking = false;
                    this.modificationsHead = [];
                    this.modificationsBody = [];
                    success.data.csv[0].forEach((item) => this.modificationsHead.push([item]))
                    for (let i = 1; i < success.data.csv.length; i++) {
                        this.modificationsBody.push(success.data.csv[i]);
                    }
        ...
        onSubmitModifications() {
            ...
            setTimeout(() => {
                ...
                for (var i = 0; i < this.csvModifications.length; i++) {
                    let file = this.csvModifications[i];
                    formData.append('files[' + i + ']', file);
                }
                axios.post('/api/csv/prepare/modifications', formData, {
                    ...
                    (success) => {
                        ...
                        this.modificationsBody = [];
                        this.modificationsLengthUploaded = 0;
                        this.csvModificationsFileName = [];
                        let uploadedFiles = this.$refs.csv_modifications.files;
                        for (var i = 0; i < uploadedFiles.length; i++) {
                            this.csvModificationsFileName.push(uploadedFiles[i].name);
                        }
                        success.data.csv[0].forEach((item) => this.modificationsHead.push([item]))
                        for (let i = 1; i < success.data.csv.length; i++) {
                            this.modificationsBody.push(success.data.csv[i]);
                        }
                        this.modificationsBody.pop();
                        setTimeout(() => {
                            this.preload = false;
                            this.dialog = true;
                            this.error = success.data.errors;
                        }, 300);
                        var file = document.getElementById('csv_modifications');
        ...
        modificationsRepeat(items) {
            let repeat = [];
            setTimeout(async () => {
                for (const item of items) {
                    if (this.disableAllFields) {
                        item.forEach(row => row.loading = 'loading');
                        await axios.post('/api/csv/upload/modification', {
                            modificationsHeader: this.modificationsHead,
                            modifications: item,
                            user: this.userId,
                        }).then(
                            (success) => {
                                ...
                                this.modificationsLengthUploaded++;
                            },
                            (error) => {
                                item.forEach(row => row.loading = 'problem');
                                repeat.push(item);
                                console.log('Не удалось загрузить модификацию, ожидайте повтора.');
                            }
                        )
                    }
                }
                if (repeat.length > 0) {
                    this.modificationsRepeat(repeat);
                }
            }, 10000);
        },
        async uploadModifications() {
            this.modificationsFinished = false;
            this.modificationsLengthUploaded = 0;
